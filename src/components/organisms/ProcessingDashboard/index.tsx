  ...
  return (
    <div className="bg-gray-800 rounded-lg p-4">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-white font-medium">{t('processing.title')}</h3>
        <button 
      ...
      <div className="space-y-3">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-gray-300">{t('processing.overallProgress')}</span>
        ...
        <div className="grid grid-cols-4 gap-4 text-center">
          <div className="bg-gray-700 rounded-lg p-3">
            ...
            <div className="text-xs text-gray-400">{t('status.completed')}</div>
          </div>
          
          <div className="bg-gray-700 rounded-lg p-3">
            <div className="text-lg font-bold text-blue-400">{processingSegments}</div>
            <div className="text-xs text-gray-400">{t('status.processing')}</div>
          </div>
          
          <div className="bg-gray-700 rounded-lg p-3">
            <div className="text-lg font-bold text-red-400">{errorSegments}</div>
            <div className="text-xs text-gray-400">{t('status.error')}</div>
          </div>
        ...
        <div className="text-sm text-gray-300">
          {totalSegments > 0 && (
            <div className="flex justify-between">
              <span>{t('processing.segments')}: {completedSegments} / {totalSegments}</span>
              <span>
                {t('session.claims')}: {segments.reduce((sum, seg) => sum + seg.claimsCount, 0)}
              </span>
            </div>
          )}
        </div>
